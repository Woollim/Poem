...
시는 혼자 쓰고 끝나는 글이 아니라 누군가에게 건네질 때 완성된다고 생각했어요. 그런데 습작을 책으로 엮어 보여줄 방법이 마땅치 않았어요. 그래서 시를 쓰고, 마음에 드는 시를 골라 표지가 있는 시집으로 엮고, 옆자리 친구에게 책을 건네듯 블루투스로 전할 수 있는 앱을 만들었어요.
...
2017년 당시 사용하던 EC2 서버는 지금 남아 있지 않아요. 그래서 당시 API를 재현한 목 서버를 `backend/server.py`로 만들었어요. 파이썬 표준 라이브러리만 사용해서 `python3 backend/server.py`로 바로 띄울 수 있고, 앱은 `RetrofitClass.BASE_URL`(기본값은 에뮬레이터 기준 `http://10.0.2.2:8081`)로 이 서버를 바라봐요.
...
├── Model/        데이터 모델
└── Util/         공통 유틸리티
backend/          당시 서버를 재현한 목 서버 (server.py)
resource/         데모 영상
```
